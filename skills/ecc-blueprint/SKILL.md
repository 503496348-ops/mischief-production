...
This skill ships with hermes-code-patterns. No separate installation is needed when HCP is installed.

### Full HCP install

If you are working from the HCP repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the HCP diff before updating:

```bash
cd /path/to/hermes-code-patterns
...
If you are vendoring only this skill outside the full HCP install, copy the reviewed file from the HCP repository into `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed HCP commit rather than running `git pull`.
