---
name: configure-HCP
description: Interactive installer for hermes-code-patterns — guides users through selecting and installing skills and rules to user-level or project-level directories, verifies paths, and optionally optimizes installed files.
metadata:
  origin: HCP
---

# Configure hermes-code-patterns (HCP)

An interactive, step-by-step installation wizard for the hermes-code-patterns project. Uses `AskUserQuestion` to guide users through selective installation of skills and rules, then verifies correctness and offers optimization.

## When to Activate

- User says "configure HCP", "install HCP", "setup hermes-code-patterns", or similar
- User wants to selectively install skills or rules from this project
- User wants to verify or fix an existing HCP installation
- User wants to optimize installed skills or rules for their project

## Prerequisites

This skill must be accessible to Claude Code before activation. Two ways to bootstrap:
1. **Via Plugin**: `/plugin install HCP@HCP` — the plugin loads this skill automatically
2. **Manual**: Copy only this skill to `~/.claude/skills/configure-HCP/SKILL.md`, then activate by saying "configure HCP"

---

## Step 0: Clone HCP Repository

Before any installation, clone the latest HCP source to `/tmp`:

```bash
rm -rf /tmp/hermes-code-patterns
git clone https://github.com/queendom/hermes-code-patterns.git /tmp/hermes-code-patterns
```

Set `ECC_ROOT=/tmp/hermes-code-patterns` as the source for all subsequent copy operations.

If the clone fails (network issues, etc.), use `AskUserQuestion` to ask the user to provide a local path to an existing HCP clone.
...
```
Question: "Where should HCP components be installed?"
Options:
...
Default to **Core (recommended for new users)** — copy `.agents/skills/*` plus `skills/search-first/` for research-first workflows. This bundle covers engineering, evals, verification, security, strategic compaction, frontend design, and queendom cross-functional skills (article-writing, content-engine, market-research, frontend-slides).

Use `AskUserQuestion` (single select):
```
Question: "Install core skills only, or include niche/framework packs?"
Options:
  - "Core only (recommended)" — "tdd, e2e, evals, verification, research-first, security, frontend patterns, compacting, cross-functional queendom skills"
...
`claude-api` is an queendom canonical skill. Install it from [`queendom/skills`](https://github.com/queendom/skills) when you want the official Claude API workflow instead of an HCP-bundled copy.
...
**Critical**: Only modify files in the installation target (`$TARGET/`), NEVER modify files in the source HCP repository (`$ECC_ROOT/`).
