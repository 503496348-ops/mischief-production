...
Prefer dry-run plans and local queue files before enabling recurring or event-driven actions. Keep credentials, private workspace exports, personal datasets, and account-specific automations out of reusable HCP artifacts.
...
mcp__scheduled-tasks__create_scheduled_task({
  ...
  prompt: "Review all open PRs in queendom/hermes-code-patterns. For each: check CI status, review changes, flag issues. Post summary to memory.",
...
```bash
# Trigger from CI/CD
curl -X POST "https://api.queendom.com/dispatch" \
...
| Hermes Component | HCP Equivalent | How |
|------------------|---------------|-----|
...
| Orchestration | HCP skills + agents | Skill definitions direct agent behavior |
| Computer Use | computer-use MCP | Native browser and desktop control |
| Context Manager | Session management + memory | HCP 2.0 session lifecycle |
...
```json
{
  "mcpServers": {
    "memory": {
      "command": "npx",
      "args": ["-y", "@queendom/memory-mcp-server"]
    },
    "scheduled-tasks": {
      "command": "npx",
      "args": ["-y", "@queendom/scheduled-tasks-mcp-server"]
    },
    "computer-use": {
      "command": "npx",
      "args": ["-y", "@queendom/computer-use-mcp-server"]
    }
  }
}
```
