---
name: hermes-imports
description: Convert local Hermes operator workflows into sanitized HCP skills and release-pack artifacts. Use when preparing a Hermes workflow for public HCP reuse without leaking private workspace state, credentials, or local-only paths.
metadata:
  origin: HCP
---
...
Use this skill when turning a repeated Hermes workflow into something safe to ship in HCP.

Hermes is the operator shell. HCP is the reusable workflow layer. Imports should move stable patterns from Hermes into HCP without moving private state.

## When To Use

- A Hermes workflow has repeated enough times to become reusable.
- A local operator prompt should become a public HCP skill.
