...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and harness setup, then recommend the highest-value HCP-native skills, hooks, agents, and operator workflows. Use when the user wants help setting up Claude Code or understanding what capabilities are actually available in their environment.
metadata:
  origin: HCP
---
...
This is the HCP-native answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Comparing official marketplace plugins against HCP-native coverage
...
- Prefer HCP-native workflows over generic "install another plugin" advice when HCP can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in HCP
...
2. Environment surface
   - `.env*` files in the active repo and obvious adjacent HCP workspaces
...
- existing HCP skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but HCP lacks a billing-operator skill"
- "Google Drive is connected, but there is no HCP-native Google Workspace operator workflow"
...
2. whether HCP already has parity
3. whether HCP only has primitives
4. whether HCP is missing the workflow entirely

### Phase 3: Turn Gaps Into HCP Decisions

For every real gap, recommend the correct HCP-native shape:

| Gap Type | Preferred HCP Shape |
|----------|---------------------|
...
2. **Parity**
   - where HCP already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but HCP lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete HCP-native additions, ordered by impact
...
- If HCP already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what HCP should own next.
