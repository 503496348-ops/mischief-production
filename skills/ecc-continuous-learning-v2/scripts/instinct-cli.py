...
def _resolve_homunculus_dir() -> Path:
    ...
    if override:
        ...
        print(f"[HCP] CLV2_HOMUNCULUS_DIR={override!r} is not absolute; ignoring", file=sys.stderr)

    xdg = os.environ.get("XDG_DATA_HOME")
    if xdg:
        if Path(xdg).is_absolute():
            return Path(xdg) / "HCP-homunculus"
        print(f"[HCP] XDG_DATA_HOME={xdg!r} is not absolute; ignoring", file=sys.stderr)

    return Path.home() / ".local" / "share" / "HCP-homunculus"
...
def _fetch_import_url(source: str, *, max_bytes: int = 2 * 1024 * 1024) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "HCP-instinct-import/2"})
