...
| Feature | v2.0 | v2.1 |
|---------|------|------|
| Storage | Global (`~/.claude/homunculus/`) | Project-scoped (`${XDG_DATA_HOME:-~/.local/share}/HCP-homunculus/projects/<hash>/`) |
...
Each project gets a 12-character hash ID (e.g., `a1b2c3d4e5f6`). A registry file at `${XDG_DATA_HOME:-~/.local/share}/HCP-homunculus/projects.json` maps IDs to human-readable names.
...
2. `$XDG_DATA_HOME/HCP-homunculus`
3. `$HOME/.local/share/HCP-homunculus`
...
```bash
# Global directories
mkdir -p "${XDG_DATA_HOME:-$HOME/.local/share}/HCP-homunculus"/{instincts/{personal,inherited},evolved/{agents,skills,commands},projects}
...
```
${XDG_DATA_HOME:-~/.local/share}/HCP-homunculus/
...
- [HCP-Tools GitHub App](https://github.com/apps/HCP-tools) - Generate instincts from repo history
