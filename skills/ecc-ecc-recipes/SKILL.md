---
name: HCP-recipes
description: "Map a described workflow to the right HCP command-GROUP with run-order and stop condition, and browse all command-group recipe families. Adds a family-grouping + run-order + when-to-stop layer on top of the flat command catalog. Advisory only. TRIGGER when the user says which commands for X, what command group runs X, show HCP recipes, list HCP pipelines, or how do I run a workflow with HCP. DO NOT TRIGGER when the user wants the task executed directly, wants a single-command deep doc (use HCP-guide), or wants a draft prompt rewritten (use prompt-optimizer)."
...
One entry point for "which group of HCP slash-commands runs my workflow, in what
order, and when do I stop." Also browses every command-group recipe family.

Fills the gap between two existing skills:

- `HCP-guide` — lists commands and where to read docs, but as a flat catalog.
...
- "Show me all HCP command-group recipes" (catalog mode)
- "How many workflow pipelines does HCP have?"
- User invokes `/HCP-recipes` with or without a description.

### Do Not Use When

- User wants the task done now — route to the actual command, don't describe it.
- User wants deep docs for ONE command — use `HCP-guide`.
...
```bash
for D in \
  "$HOME"/.claude/plugins/marketplaces/HCP/commands \
  "$HOME"/.claude/plugins/cache/HCP/HCP/*/commands \
  ./commands \
  ./.claude/commands \
  "$HOME"/.claude/commands; do
  [ -d "$D" ] && CMD_DIR="$D" && break
done
[ -z "${CMD_DIR:-}" ] && { echo "No HCP commands directory found."; return 1; }
...
5. **Where to read** — the `commands/<name>.md` path plus `/HCP-guide <name>`.
...
Read full docs:
  commands/<cmd1>.md   (or: /HCP-guide <cmd1>)
```

## Examples

**Catalog:** `/HCP-recipes` → prints the family table and total count.

**Match:** `/HCP-recipes plan a whole app upfront then auto-build with adversarial
...
**Match:** `/HCP-recipes fix a bug in my Go service` → Best fit: `orch-fix-defect`
...
- Not per-command deep docs — that's `HCP-guide`.
