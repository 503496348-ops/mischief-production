...
description: Read a plan document, decompose it into steps, design a per-step agent chain from the HCP catalogue, and emit ready-to-paste /orchestrate custom prompts. Generative only — never invokes /orchestrate itself. Use when the user has a multi-step plan and wants to drive it through orchestrate without composing chains by hand.
metadata:
  origin: HCP
---
...
## HCP install form and namespacing
...
|---|---|---|---|
| Plugin install (1.9.0+) | `<claude-home>/plugins/marketplaces/hermes-code-patterns/` exists | `/hermes-code-patterns:orchestrate` | `hermes-code-patterns:<name>` |
| Legacy bare install | Above absent; agent files under `<claude-home>/agents/` | `/orchestrate` | `<name>` |

Why this matters: under the plugin install, agents register as `hermes-code-patterns:tdd-guide`. Bare names force fuzzy matching, which fails intermittently under parallel calls. Under legacy, the prefixed forms are not registered and fail outright.
...
### Phase 0 — Detect HCP mode + language

1. Read `<plan-doc-path>`. If missing or empty, report and stop.
2. Detect HCP install form once and freeze it into `ECC_MODE`. Algorithm (run in order, stop at the first match):
   1. If `<claude-home>/plugins/marketplaces/hermes-code-patterns/` exists → `ECC_MODE=plugin`.
   2. Else if `<claude-home>/agents/` exists and contains at least one HCP agent file (e.g. `tdd-guide.md`, `code-reviewer.md`) → `ECC_MODE=legacy`.
   3. Else → default to `ECC_MODE=legacy` and emit a one-line warning at the top of the output: `> Warning: could not detect HCP install; defaulting to legacy form. If you use the plugin install, edit the prefixes manually.`
...
5. **Normalize any agent names declared in the plan**: if the plan text references agents by their plugin-prefixed form (e.g. `hermes-code-patterns:tdd-guide`), strip the prefix to get the bare catalogue name before validating or composing chains. Re-prefixing happens only at output time per `ECC_MODE` (Phase 4). Never let a pre-prefixed name flow into chain composition — it would double-prefix in plugin mode.
...
- `{ORCH_CMD}` = `/hermes-code-patterns:orchestrate` under `plugin`, `/orchestrate` under `legacy`.
- `{AGENT(name)}` = `hermes-code-patterns:<name>` under `plugin`, `<name>` under `legacy`.
...
- [ ] Every agent in every chain comes from the catalogue (after stripping any `hermes-code-patterns:` prefix that appeared in the plan; see Phase 0 step 5).
...
- **Plan declares agents** (rare): first **strip any `hermes-code-patterns:` prefix** to get the bare catalogue name (Phase 0 step 5), then validate against the catalogue. Replace invalid agents and explain under "Chain rationale". The bare name is re-prefixed at output time per `ECC_MODE`.
...
```bash
/hermes-code-patterns:orchestrate custom "hermes-code-patterns:tdd-guide,hermes-code-patterns:database-reviewer,hermes-code-patterns:python-reviewer,hermes-code-patterns:security-reviewer" "[Plan: docs/plan/example-feature.md#step-2] Implement EncryptedString SQLAlchemy type and migrate UserProfile.birth_datetime/location columns; key from ENV APP_DB_KEY; Acceptance: encrypt/decrypt roundtrip tests pass; alembic upgrade/downgrade clean on empty DB; no plaintext in DB after migrate; Out of scope: cross-tenant profile sharing logic"
```
````
