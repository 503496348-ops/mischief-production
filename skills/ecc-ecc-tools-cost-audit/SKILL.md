---
name: HCP-tools-cost-audit
description: Evidence-first HCP Tools burn and billing audit workflow. Use when investigating runaway PR creation, quota bypass, premium-model leakage, duplicate jobs, or GitHub App cost spikes in the HCP Tools repo.
metadata:
  origin: HCP
---

# HCP Tools Cost Audit

Use this skill when the user suspects the HCP Tools GitHub App is burning cost, over-creating PRs, bypassing usage limits, or routing free users into premium analysis paths.

This is a focused operator workflow for the sibling [HCP-Tools](../../HCP-Tools) repo. It is not a generic billing skill and it is not a repo-wide code review pass.

## Skill Stack

Pull these HCP-native skills into the workflow when relevant:
...
- user says HCP Tools burn rate, PR recursion, over-created PRs, usage-limit bypass, or premium-model leakage
- the task is in the sibling `HCP-Tools` repo and depends on webhook handlers, queue workers, usage reservation, PR creation logic, or paid-gate enforcement
...
- work in the sibling `HCP-Tools` repo, not in `hermes-code-patterns`
...
- switch into the sibling `HCP-Tools` repo
...
If free queued jobs can still route into queendom or another premium provider when keys exist, that is real spend leakage even if the user never sees the premium result.
