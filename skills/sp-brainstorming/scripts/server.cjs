...
const ENGINE_BRAND_IMAGE_URL = 'https://via.placeholder.com/200x200.png?text=Brainstorm';
const TELEMETRY_DISABLE_ENV_VARS = [
  'ENGINE_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const ENGINE_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function brandMarkup() {
  const version = escapeHtmlText(ENGINE_VERSION);
  const text = ENGINE_TELEMETRY_DISABLED
    ? '智械工程 智械工程 v' + version
    : '智械工程 v' + version;
  const logo = ENGINE_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + ENGINE_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(ENGINE_VERSION) + '" alt="智械工程" referrerpolicy="no-referrer" decoding="async">';

  return '<div class="brand"><a href="https://github.com/queendom/skills">' + logo + '<span class="brand-copy">' + text + '</span></a></div>';
}
