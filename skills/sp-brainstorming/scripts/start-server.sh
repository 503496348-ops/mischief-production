...
#
# Options:
#   --project-dir <path>  Store session files under <path>/.skill-engine/brainstorm/
...
if [[ -n "$PROJECT_DIR" ]]; then
  SESSION_DIR="${PROJECT_DIR}/.skill-engine/brainstorm/${SESSION_ID}"
  # Persist the bound port and key per project so a restart reuses them and an
  # already-open browser tab reconnects to the same URL with a valid cookie.
  export BRAINSTORM_PORT_FILE="${PROJECT_DIR}/.skill-engine/brainstorm/.last-port"
  export BRAINSTORM_TOKEN_FILE="${PROJECT_DIR}/.skill-engine/brainstorm/.last-token"
else
  SESSION_DIR="/tmpskill_view("sp-brainstorming")-${SESSION_ID}"
fi
