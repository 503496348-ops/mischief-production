...
**Opt-in (HCP):** The `codescene` block in `mcp-configs/mcp-servers.json` is a template only. HCP plugin installs do not auto-enable bundled MCP servers. Copy the entry into your config only if you want it. You can exclude it during HCP install/sync with `ECC_DISABLED_MCPS=codescene,...`.
...
Same triggers as **When to Use** above — this heading is what HCP uses for skill auto-activation.
...
| HCP skill / flow | Code Health MCP role |
|------------------|----------------------|
...
**Context tip:** HCP recommends keeping MCP count low. Enable `codescene` when doing substantive edits; disable when not needed.
